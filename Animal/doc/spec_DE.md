...
Globale Beschreibung: **Eine Beobachtung von Tierzuständen an einem bestimmten Ort und zu einer bestimmten Zeit. Dieses Datenmodell wurde für das IoF2020 UC ShareBeef von UCO und SensoWave entwickelt.**  

## Liste der Eigenschaften  

- `address`: Die Postanschrift  - `alternateName`: Ein alternativer Name für diesen Artikel  - `areaServed`: Das geografische Gebiet, in dem eine Dienstleistung oder ein angebotener Artikel erbracht wird  - `birthdate`: Geburtsdatum des Tieres  - `breed`: Rasse des Tieres  - `calvedBy`: Mutter des Tieres  - `dataProvider`: Eine Folge von Zeichen, die den Anbieter der harmonisierten Dateneinheit identifiziert.  - `dateCreated`: Zeitstempel der Entitätserstellung. Dieser wird normalerweise von der Speicherplattform zugewiesen.  - `dateModified`: Zeitstempel der letzten Änderung der Entität. Dieser wird in der Regel von der Speicherplattform vergeben.  - `description`: Eine Beschreibung dieses Artikels  - `fedWith`: Für das Tier verwendete Lebensmittel  - `healthCondition`: Phänologischer Zustand des Tieres. Enum:' gesund, krank, inBehandlung'  - `id`: Eindeutiger Bezeichner der Entität  - `legalId`: Legale ID des Tieres  - `locatedAt`: Id der AgriParcel-Beziehung  - `location`: Geojson-Referenz auf das Element. Es kann Punkt, LineString, Polygon, MultiPoint, MultiLineString oder MultiPolygon sein  - `name`: Der Name dieses Elements.  - `ownedBy`: Der Besitzer des Tieres  - `owner`: Eine Liste mit einer JSON-kodierten Zeichenfolge, die auf die eindeutigen Ids der Eigentümer verweist  - `phenologicalCondition`: Phänologischer Zustand des Tieres. Enum:'lactatingBaby, grazingBaby, maleAdult, femaleAdult, maleYoung, femaleYoung'.  - `relatedSource`: Liste der IDs, die die aktuelle Entität in externen Anwendungen haben kann  - `reproductiveCondition`: Fortpflanzungszustand des Tieres. Enum:'noStatus, inactive, inCalf, inHeat, active'  - `seeAlso`: Liste von uri, die auf zusätzliche Ressourcen über das Element verweist  - `sex`: Geschlecht des Tieres. Enum:'männlich, weiblich'  - `siredBy`: Vater des Tieres  - `source`: Eine Folge von Zeichen, die die ursprüngliche Quelle der Entitätsdaten als URL angibt. Empfohlen wird der voll qualifizierte Domänenname des Quellanbieters oder die URL zum Quellobjekt.  - `species`: Spezies, zu der das Tier gehört. Diese Aufzählung kann erhöht werden  - `type`: NGSI-Entitätstyp: Es muss Tier sein  - `weight`: Das Gewicht des Tieres als Zahl  - `welfareCondition`: Indikator für das Tierwohl. Enum:'issue, adequate'    
Erforderliche Eigenschaften  
- `id`  - `legalId`  - `sex`  - `species`  - `type`    
Das vorgeschlagene Tierdatenmodell wurde von einem allgemeineren Standpunkt aus erstellt und versucht, es an die Informationen anzupassen, die von den in der UC verwendeten Geräten und Sensoren stammen. Das nächste Diagramm beschreibt die Rindfleischkette. In diesem Diagramm werden verschiedene Akteure der Fleischkette sowie einige ihrer Interaktionen beschrieben. ![ ](../resources/diagramm1.jpg). Während der Verwendung dieses Datenmodells wird es notwendig sein, mehrere Entitäten zu definieren, um die in der vorgeschlagenen Lösung generierten Informationen zu verarbeiten. Innerhalb all dieser Entitäten sticht in erster Linie die Tier-Entität hervor, die das Zentrum der Lösung ist  
...
Hier ist ein Beispiel für ein Tier im JSON-LD-Format als Key-Values. Dies ist kompatibel mit NGSI-v2 bei Verwendung von `options=keyValues` und liefert die Kontextdaten einer einzelnen Entität.  
...
Hier ist ein Beispiel für ein Tier im JSON-LD-Format in normalisierter Form. Dies ist kompatibel mit NGSI-v2, wenn keine Optionen verwendet werden, und gibt die Kontextdaten einer einzelnen Entität zurück.  
...
Hier ist ein Beispiel für ein Tier im JSON-LD-Format als Key-Values. Dies ist kompatibel mit NGSI-LD bei Verwendung von `options=keyValues` und gibt die Kontextdaten einer einzelnen Entität zurück.  
...
Hier ist ein Beispiel für ein Tier im JSON-LD-Format in normalisierter Form. Dies ist kompatibel mit NGSI-LD, wenn keine Optionen verwendet werden, und gibt die Kontextdaten einer einzelnen Entität zurück.  
